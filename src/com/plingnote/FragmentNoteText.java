...
public class FragmentNoteText extends Fragment {
	...
	public void onPause (){
		...
		if(isExisting && (getTitleofNoteText().length() >0 || getTextofNoteText().length() > 0)){
			DatabaseHandler.getInstance(this.getActivity()).updateNote(this.id,this.getTitleofNoteText(), this.getTextofNoteText(), null,null,null,NoteCategory.NO_CATEGORY,null);
		}
		//If this class not was opened with an intent o saved instance we are inserting the note in database.
			...
			if(getTitleofNoteText().length() >0 || getTextofNoteText().length() > 0){
				DatabaseHandler.getInstance(this.getActivity()).insertNote(this.getTitleofNoteText(), this.getTextofNoteText(), null,null,null,NoteCategory.NO_CATEGORY,null);
